...
const sendNotification = async(address, name, apy) => {
  try {
    const apiResponse = await PushAPI.payloads.sendNotification({
      ...
      notification: {
        title: `[SDK-TEST] Current Best Pool: ${name}`,
        body: `[sdk-test] Contract: ${address} with APY: ${apy}%`
      },
      payload: {
        title: `[sdk-test] Current Best Pool: ${name}`,
        body: `The contract: ${address} has the best APY at: ${apy}%`,
        cta: 'Check it now',
...
try {
  ...
  updateContractValues(bestOne.address, bestOne.name, apy, decimal);
  sendNotification(bestOne.address, bestOne.name, bestOne.apy);
} catch (error) {
